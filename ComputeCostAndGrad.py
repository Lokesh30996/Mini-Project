...
class ComputeCostAndGrad:
    ...
    def forwardPass(self, model, tree):
        ...
        self.loss += -1*label_vector.dot(np.log(tree.prediction))# Error calculation
    def backwardPass(self, model, tree, dJ_dz_prop):
        ...
        if tree.is_leaf():
            ...
            # Compute the down layer dJ_dz^1
    ...
    def calculateTotalGradient(self, model):
        ...
        if model.use_tensor:
            ...
            grad = utils.vectorizeParams(self.dJ_dWs, self.dJ_dL, self.dJ_dW, self.dJ_dV)
        else:
    ...
    def composition(self, model, child1, child2):
        ...
        if model.use_tensor:
            word_vector += c_vector.T.dot(model.V.T).dot(c_vector)#(b,c)T.V.(b,c)
        return word_vector
